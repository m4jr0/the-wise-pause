...
function onGreetingsPopupFill (_, popup) {
  ...
  introParagraph.className = `${Context.CSS_PREFIX}intro`
  ...
  fullQuoteParagraph.className = `${Context.CSS_PREFIX}full-quote`
  ...
  quoteSpan.className = `${Context.CSS_PREFIX}quote`
...
function onGreetingsPopupClosed (_) {
  quit()
}
...
function showGreetings () {
  showPopup({
    ...
    onPopupClosed: onGreetingsPopupClosed,
    closeBtnLabel: generateReply(),
    classes: `${Context.CSS_PREFIX}greetings ${Context.CSS_PREFIX}fadable`
  })
}

function makeAllGreetingsInvisible () {
  const allGreetings = document.querySelectorAll(
    `.${Context.CSS_PREFIX}greetings`
  )

  for (const greetings of allGreetings) {
    greetings.classList.add(`${Context.CSS_PREFIX}faded`)
  }
}

function makeAllGreetingsVisible () {
  const allGreetings = document.querySelectorAll(
    `.${Context.CSS_PREFIX}greetings`
  )

  for (const greetings of allGreetings) {
    greetings.classList.remove(`${Context.CSS_PREFIX}faded`)
  }
}
