...
function isNonEmptyArray (variable) {
  return Array.isArray(variable) && variable.length >= 1
}

function scrollToEnd (element, isVertical = true) {
...
function animateElementSize (
  ...
  const intervalId = setInterval(() => {
    ...
    if (
      Math.abs(currentPaddingTop - targetPaddingTop) < Math.abs(paddingTopDelta)
    ) {
...
function addElementSmoothly (
  ...
  targetMarginBottom,
  durationMs = 1000,
  refreshRateHz = Context.EFFECTS_REFRESH_RATE_HZ
) {
...
function removeElementSmoothly (
  element,
  durationMs = 1000,
  refreshRateHz = Context.EFFECTS_REFRESH_RATE_HZ
) {
